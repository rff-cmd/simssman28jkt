...
<script>
	...
	function detailvalue(id, cols){		
		...
		for(b=1; b<=cols; b++) {
			window['n'+b] = 0;	
			window['n'+b] = document.getElementById('n'+b+id).value; 
			window['n'+b] = window['n'+b].replace(/[^\d-.]/g,"");
			
			if( window['n'+b] != "" ) {
				jml_col++;
			}
			
			if(window['n'+b] == "") {window['n'+b] = 0};
			
			total_n = parseFloat(total_n) + parseFloat(window['n'+b]);
			
			if(nilai_tertinggi <  window['n'+b]) {
				nilai_tertinggi = window['n'+b];
			}
...
<script>
    function submitForm(tipe)
    {
		...
		if(tipe == 'excel_tingkat') {
			$("#daftarnilai_input").attr('action', '../app/daftarnilai_tingkat_xls.php')
			   .attr('target', '_BLANK');
			$("#daftarnilai_input").submit();
		}
		
		if(tipe == 'upload_tingkat') {
			$("#daftarnilai_input").attr('action', '../app/daftarnilai_tingkat_upload.php')
			   .attr('target', '_BLANK');
			$("#daftarnilai_input").submit();
		}
...
<div class="page-content">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="daftarnilai_input" id="daftarnilai_input" enctype="multipart/form-data" onSubmit="return cekinput('date,client_code,cheque_no,account_name,bank_name,bank_branch');" >
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="submit" name="submit" class='btn btn-primary' value="Download" onclick="submitForm('excel_tingkat')" >
					</div>
				</div>
				
				<?php if($protection->setup_periode("RAPORT", $date) == 1 || allowlvl("frmdaftarnilai") == 2) { ?>
					<div class="form-group">
				...
				<?php if( allowlvl("frmdaftarnilai") == 2) { ?>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Upload File per Tingkat</label>
						<div class="col-sm-3">
							<input type="file" name="file" id="file" accept=".csv">
							<br>
							<input type="submit" name="submit" class='btn btn-primary' value="Upload per Tingkat" onclick="submitForm('upload_tingkat')" >
						</div>
					</div>
				<?php } ?>
					
				<table class="table" id="simple-table" class="table table-striped table-bordered table-hover">
					<thead>
						<?php
					...
					<tbody>
                        <?php			
							...
							$i++;
								...
								$sqlnilai 	= $select->list_daftarnilai($siswa_view->replid, $idtingkat, $idkelas, "", $semester_id, $idpelajaran, $iddasarpenilaian);
								$siswa_get	= $sqlnilai->fetch(PDO::FETCH_OBJ);
								
								$kelas = $siswa_view->tingkat . " / " . $siswa_view->kelas;
						?>
                                ...
                                <tr style="font-size: 11px">
									...
									?>
											<td><input type="text" style="width:50px; height: 30px; text-align: center;" name="n<?php echo $b.$i ?>" id="n<?php echo $b.$i ?>" onkeyup="formatangka('n<?php echo $b.$i ?>'),detailvalue('<?php echo $i ?>', '<?php echo $jumlah_ukbm ?>')" value="<?php echo $siswa_get->n1 ?>"/></td>
											
											<input type="hidden" name="line_det<?php echo $b.$i ?>" id="line_det<?php echo $b.$i ?>" value="<?php echo $b ?>"/>
									<?php
											}
										} else {
											$b = 1;
									...
									?>
											...
											<td><input type="text" style="width:50px; height: 30px; text-align: center;" name="n<?php echo $b.$i ?>" id="n<?php echo $b.$i ?>" onkeyup="formatangka('n<?php echo $b.$i ?>'),detailvalue('<?php echo $i ?>', '<?php echo $jumlah_ukbm ?>')" value="<?php echo $datanilai_detail->nilai ?>"/></td>
											
											<input type="hidden" name="line_det<?php echo $b.$i ?>" id="line_det<?php echo $b.$i ?>" value="<?php echo $datanilai_detail->line ?>"/>
									...
									<input type="hidden" style="width:50px; height: 30px; text-align: center;"  name="uts<?php echo $i ?>" id="uts<?php echo $i ?>" onkeyup="formatangka('uts<?php echo $i ?>'),detailvalue('<?php echo $i ?>', '<?php echo $jumlah_ukbm ?>')" value="<?php echo $siswa_get->uts ?>"/>
									...
									<?php if(allowlvl("frmdaftarnilai") == 2) { ?>
										<td><input type="text" style="width:50px; height: 30px; text-align: center;"  name="uas<?php echo $i ?>" id="uas<?php echo $i ?>" onkeyup="formatangka('uas<?php echo $i ?>'),detailvalue('<?php echo $i ?>', '<?php echo $jumlah_ukbm ?>')" value="<?php echo $siswa_get->uas ?>"/></td>
									<?php } else { ?>
									...
									<td id="na_<?php echo $i ?>" ><input type="text" style="width:50px; height: 30px; background-color: #bbfaab; text-align: center;"  name="na<?php echo $i ?>" id="na<?php echo $i ?>" readonly value="<?php echo number_format($siswa_get->na,0,'.',',') ?>"/></td>

	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='pegawai_id') {
			alert('Pembimbing Akademik tidak boleh kosong!');				
		  }
...
<script type="text/javascript">
	...
	function loadHTMLPost2(URL, destination, button, getId, getId1){
		dest = destination;	
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$act = "checked";
				if ($ref != "") {
					$sql=$select->list_siswa_krs($ref);
					$row_siswa_krs=$sql->fetch(PDO::FETCH_OBJ);	
					$idkelas = $row_siswa_krs->idkelas;
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="siswa_krs" id="siswa_krs" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('nis,semester_id,pegawai_id');" >
            	...
            	<div class="form-group">
					...
					<div class="col-sm-4">
						<select name="nis" id="nis" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_siswa_all($row_siswa_krs->nis); ?>
						</select>
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-lg-2">
						<select name="semester_id" id="semester_id" class="chosen-select form-control" onChange="loadHTMLPost2('app/siswa_krs_ajax.php','siswa_krs_pelajaran','getkrs','nis','semester_id')" >
							<option value=""></option>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<select name="pegawai_id" id="pegawai_id" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_petugas($pegawai_id); ?>
						</select>
					</div>
				</div>
                
            	<div id="siswa_krs_pelajaran">
            		<?php
            			if( !empty($ref) ) {
            				$sqlsiswa = $select->list_siswa($row_siswa_krs->nis);
							$datasiswa = $sqlsiswa->fetch(PDO::FETCH_OBJ);
							$idminat = $datasiswa->idminat;
							$idtingkat = $datasiswa->idtingkat;
							
							include("siswa_krs_pelajaran_approved_update.php");
						}
            		?>
            	</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('siswa_krs_approved')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('siswa_krs_approved')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php /* if (allowdel('frmsiswa_krs')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('siswa_krs_approved_view') ?>'" />

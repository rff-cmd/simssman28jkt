...
function GoToLastMonth() {
	document.location.href = "cals.php?bulan=<?php echo $last_month?>&tahun=<?php echo $last_year?>&elementid=<?php echo $elementid?>";
}

function GoToNextMonth() {
	document.location.href = "cals.php?bulan=<?php echo $next_month?>&tahun=<?php echo $next_year?>&elementid=<?php echo $elementid?>";
}

function ChangeCal() {
	var bulan = document.getElementById('bulan').value;
	var tahun = document.getElementById('tahun').value;
	document.location.href = "cals.php?elementid=<?php echo $elementid?>&bulan=" + bulan + "&tahun=" + tahun;
}
...
<?php $namabulan = array("Jan","Feb","Mar","Apr","Mei","Jun","Jul","Ags","Sep","Okt","Nov","Des");
   ...
   for ($i = 1; $i <= 12; $i++) { ?>
	<option value="<?php echo $i?>" <?php echo IntIsSelected($i, $bulan)?>><?php echo $namabulanpjg[$i - 1]?></option>
<?php } ?>
</select>    
<select name="tahun" class="cmbfrm" id="tahun" onchange="ChangeCal()">
<?php $YNOW = date('Y');
   for ($i = 1900; $i <= $YNOW + 1; $i++) { ?>
	<option value="<?php echo $i?>" <?php echo IntIsSelected($i, $tahun)?>><?php echo $i?></option>
<?php } ?>
</select>    
...
<table class="table" border="0" cellpadding="5" cellspacing="1" width="100%" style="border-color:#999999">

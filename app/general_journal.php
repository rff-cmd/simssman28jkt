...
<script type="text/javascript">
	...
	function loadHTMLPost4(URL, destination, button, getId, getId2, getId3, getId4){
		dest = destination;	
		str = getId + '=' + document.getElementById(getId).value;		
		str2 = getId4 + '=' + document.getElementById(getId4).value;
...
<script>
	function print() {
		...
		window.open('app/general_journal_print.php?ref='+ref, '','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
	...
	<div class="row">
		<div class="col-xs-12">
			<?php 
				...
				include("app/exec/insert_general_journal.php"); 
				
				//jika saat add data, maka data setelah save kosong
				if ($_POST['submit'] == 'Save') { $ref = ''; }
				//-----------------------------------------------/\
				
				$ref2 = notran(date('y-m-d'), 'frmgeneral_journal', '', '', ''); //---get no ref
				
				if ($ref != "") {
					$sql=$select->list_general_journal($ref);			
					$row_general_journal=$sql->fetch(PDO::FETCH_OBJ);	
					
					$ref2 = $row_general_journal->ref;			
			...
			<form class="form-horizontal" role="form" action="" method="post" name="general_journal" id="general_journal" enctype="multipart/form-data" onSubmit="return cekinput('ref,date');" />
				...
				<div class="form-group"> 
					...
					<div class="col-sm-2">
						<input class="form-control date-picker" data-date-format="dd-mm-yyyy" type="text" id="date" name="date" value="<?php if($row_general_journal->date !='' ){echo date('d-m-Y', strtotime($row_general_journal->date)); } else { echo date('d-m-Y');} ?>"></div>
				</div>
				...
				<div class="form-group"> 
					...
					<div class="col-sm-3">
						<select name="status" id="status" class="chosen-select form-control" style="min-width: 178px;" />
							<?php general_journal_status($row_general_journal->status); ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group"> 
					...
					<div class="col-sm-3">
						<input align="right" type="text" id="rate" name="rate" onkeyup="formatangka('excrte')" value="<?php echo number_format($row_general_journal->rate,0,".",",") ?>" style="text-align:right" >
                    </div>
				</div>
				...
				<div class="form-group"> 
					...
					<div class="col-sm-5">
						<textarea id="mmo" name="mmo" rows="3" cols="50" ><?php echo $row_general_journal->mmo ?></textarea>
                    </div>
				</div>
				*/ ?>
				...
				<div class="form-group"> 
					...
					<div class="col-sm-3">
						<input style="background-color:#E2F6C5;" type="text" readonly id="uid" name="uid" value="<?php echo $row_general_journal->uid ?>">
					</div>
				</div>
				...
				<div class="form-group"> 
					...
					<div class="col-sm-3">
						<input style="background-color:#E2F6C5;" type="text" readonly id="dlu" name="dlu" value="<?php echo $row_general_journal->dlu ?>">
					</div>
				</div>

				<?php
					if($ref == '') {
						include("general_journal_detail.php");
					} else {
						include("general_journal_detail.php");
					}
				?>
				...
				<div class="clearfix form-actions">
						...
						<?php if (allowadd('frmgeneral_journal','')==1) { ?>
							<?php if($ref=='') { ?>
						...
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('general_journal_view') ?>'" />

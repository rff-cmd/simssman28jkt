<?php
	$sql=$select->list_item_group_detail($ref);
	$jmldata = $sql->rowCount();
?>

<table class="table" class="table table-bordered table-condensed table-hover table-striped" style="width: auto;">
	<thead>
		<tr> 
			...
			<th><?php if($lng==1) { echo 'Purchase Discount Account'; } else { echo 'Akun Diskon Pembelian'; } ?></th>
			<th><?php if($lng==1) { echo 'Work in Process Acc'; } else { echo 'Akun Proses Produksi'; } ?></th>
			<th><?php if($lng==1) { echo 'Consignment Account'; } else { echo 'Akun Konsinyasi'; } ?></th>					
	...
	<tbody>
		...
		<?php 
			$no = 0;
			while($row_item_group_detail=$sql->fetch(PDO::FETCH_OBJ)) { 		
		?>
				
				<input type="hidden" id="old_line_<?php echo $no ?>" name="old_line_<?php echo $no ?>" value="<?php echo $row_item_group_detail->line; ?>" >
				<input type="hidden" id="old_id_<?php echo $no ?>" name="old_id_<?php echo $no ?>" value="<?php echo $row_item_group_detail->id; ?>" >
				...
				<tr style="background-color:ffffff;" > 	
					<td>
						<select id="location_id_<?php echo $no ?>" name="location_id_<?php echo $no ?>" class="chosen-select form-control" style="width: 200px; font-size: 11px" >
							<option value=""></option>
							<?php 
								combo_select_active("warehouse","id","name","active","1",$row_item_group_detail->location_id)
							?>	
					...
					<td>
						<select id="inventory_acccode_<?php echo $no ?>" name="inventory_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->inventory_acccode);
							?>	
					...
					<td>
						<select id="cogs_acccode_<?php echo $no ?>" name="cogs_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->cogs_acccode);
							?>	
					...
					<td>
						<select id="goodintransit_acccode_<?php echo $no ?>" name="goodintransit_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->goodintransit_acccode);
							?>	
					...
					<td>
						<select id="purchase_discount_acccode_<?php echo $no ?>" name="purchase_discount_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->purchase_discount_acccode);
							?>	
					...
					<td>
						<select id="workinprocess_acccode_<?php echo $no ?>" name="workinprocess_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->workinprocess_acccode);
							?>	
					...
					<td>
						<select id="consignment_acccode_<?php echo $no ?>" name="consignment_acccode_<?php echo $no ?>" class="chosen-select form-control" style="width: auto; font-size: 11px" >
							<option value=""></option>
							<?php 
								select_coa($row_item_group_detail->consignment_acccode);
							?>	

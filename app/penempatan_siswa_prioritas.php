	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='penempatan_siswa_prioritas') {
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_penempatan_siswa_prioritas.php");
				
				$aktif = "checked";
				if ($ref != "") {
					$sql=$select->list_penempatan_siswa_prioritas($ref);
					$row_penempatan_siswa_prioritas=$sql->fetch(PDO::FETCH_OBJ);	
					
					$aktif = "";
					if ($row_penempatan_siswa_prioritas->aktif == 1) {
						$aktif = "checked";
					} else {
						$aktif = "";
					}
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="penempatan_siswa_prioritas" id="penempatan_siswa_prioritas" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('nama');" >
            	
            	<input type="hidden" id="id" name="id" value="<?php echo $ref ?>" >
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Nama Prioritas *)</label>
					<div class="col-sm-3">
						<input type="text" id="nama" name="nama" class="form-control" value="<?php echo $row_penempatan_siswa_prioritas->nama ?>">
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-3">
						<input class="ace" type="checkbox" name="aktif" id="aktif" value="1" <?php echo $aktif ?>/><span class="lbl"></span>
	            ...
	            <div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="urutan" name="urutan" class="form-control" value="<?php echo $row_penempatan_siswa_prioritas->urutan ?>">
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmpenempatan_siswa_prioritas')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmpenempatan_siswa_prioritas')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmpenempatan_siswa_prioritas')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('penempatan_siswa_prioritas_view') ?>'" />

<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('sales_order_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_sales_order($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('sales_order_view') ?>" />
			<?php
						...
						<div class="form-group">
		                    ...
		                    <div class="col-sm-3">
		                    	 <select id="status" name="status" class="chosen-select form-control">
		                    	 	<option value=""></option>
		                          	<?php 
		                                select_status_sales_order($status) 
		                            ?>	
						...
						<table class="table" id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
                                    ...
                                    <th><?php if($lng==1) { echo 'Ref No'; } else { echo 'No. Press'; } ?></th>
									<th><?php if($lng==1) { echo 'Date'; } else { echo 'Tanggal'; } ?></th>
									<th><?php if($lng==1) { echo 'Operator'; } else { echo 'Petugas'; } ?></th>
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$selectview->rpt_counting($kode, $from_date, $to_date, $client_code, $status, $all);
						            while($row_sales_order=$sql->fetch(PDO::FETCH_OBJ)){
            						
            						$i++;
            						
            						$status = "";
									if ($row_sales_order->status == "P") {
										$status = "Planned";
									}
									if ($row_sales_order->status == "R") {
										$status = "Released";
									}
									if ($row_sales_order->status == "V") {
										$status = "Verification";
									}
									if ($row_sales_order->status == "N") {
										$status = "Printing";
									}
									if ($row_sales_order->status == "C") {
										$status = "Closed";
									}
								...
								?>
                                        ...
                                        <tr>
						                    ...
						                    <td><?php echo $row_sales_order->date ?></td>
											<td><?php echo $row_sales_order->uid ?></td>
											<td><?php echo $row_sales_order->client_name ?></td>
											<td><?php echo date("H:i:s", strtotime($row_sales_order->dlu)) ?></td>
											<td align="center"><?php echo number_format($row_sales_order->total_qty,0,'.',',') ?></td>
                                            <td>
                                            
                                                <a href="javascript:void(0);" name="Find" title="Detail" onClick=window.open("<?php echo $__folder ?>app/rpt_counting_detail.php?ref=<?php echo $row_sales_order->ref ?>","Find","width=900,height=500,left=200,top=20,toolbar=0,status=0,scroll=1,scrollbars=no"); />
													<span class="green">

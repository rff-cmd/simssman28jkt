		...
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="sales" id="sales" class="form-horizontal" enctype="multipart/form-data" >
            	...
            	<div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <input type="text" id="item_code" name="item_code" style="font-size: 12px" class="form-control" oninput="loadHTMLPost2('app/rpt_stock_opname_value_ajax.php', 'item_name_id', 'get_itemname', 'item_code')" autocomplete="off" autofocus="" value="<?php echo $item_code ?>">
						...
						<table class="table" id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
                                    ...
                                    <th><?php if($lng==1) { echo 'Item Barcode'; } else { echo 'Barcode Barang'; } ?></th>
                                    <th><?php if($lng==1) { echo 'Item Name'; } else { echo 'Nama Barang'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Qty SO'; } else { echo 'Qty SO'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Stock Qty'; } else { echo 'Qty Stok'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Qty Selisih'; } else { echo 'Qty Selisih'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Unit Cost'; } else { echo 'Harga'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Amount'; } else { echo 'Total'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Amount Selisih'; } else { echo 'Total Selisih'; } ?></th>
			                    	<th></th>
							...
							<tbody>
                                <?php			
									...
									$sql=$selectview->rpt_stock_opname_value($item_code, $from_date, $to_date, $location_id, $all);			
									...
									while ($row_sales=$sql->fetch(PDO::FETCH_OBJ)) {
										...
										//get last unit cost
										$sqlcost = $selectview->list_purchase_invoice_last_discount($row_sales->item_code, $row_sales->uom_code);
										$datacost = $sqlcost->fetch(PDO::FETCH_OBJ);
										$discount1 = $datacost->discount1;
										
										$discount = ($discount1 * $datacost->unit_cost)/100;
										$unit_cost = $datacost->unit_cost - $discount;
										//------------/\--------------
										
										$total	=	$row_sales->qty * $unit_cost; //$row_sales->unit_cost;
										
										$grand_total = $grand_total + $total;
										$grand_qty = $grand_qty + $row_sales->qty;
										
										
										##get stok akhir
										/*$item_group_id2 = "";
										$date_from = $from_date; //date("Y-m-d");
										$date_to = $to_date; //date("Y-m-d");
										$sql1 = $selectview->rpt_stock($row_sales->syscode, "", $row_sales->uom_code, "", $item_group_id2, "", $date_from, $date_to);	
										$row_bincard=$sql1->fetch(PDO::FETCH_OBJ);*/
										
													
										$sql3 = $selectview->rpt_bincard_openblc_item($row_sales->syscode, $row_sales->uom_code, "", $to_date, "");
										...
										$sql1=$selectview->rpt_stock_stock_opname_limit1($row_sales->syscode, "", $row_sales->uom_code, "", $item_group_id2, "", $date_from, $to_date, "stockopname");
										$data1=$sql1->fetch(PDO::FETCH_OBJ);
										$invoice_type=$data1->invoice_type;
										if($invoice_type == "stockopname") {
											$sql2=$selectview->rpt_check_stock_opname($row_sales->syscode, "", $row_sales->uom_code, "", $item_group_id2, "", $date_from, $to_date);
											...
											$sql_so = $selectview->rpt_stock_non_so_value($row_sales->syscode, "", $row_sales->uom_code, "", $item_group_id2, "", $date_stok_opname, $to_date);	
										...
										$qty_akhir 		=	$total_qty;
										if($qty_akhir < 0) {
											$qty_selisih	=	$qty_akhir + $row_sales->qty;
										}
										if( ($qty_akhir > 0) && ($qty_akhir > $row_sales->qty) ) {
											$qty_selisih	=	$qty_akhir - $row_sales->qty;
										}
										if( ($qty_akhir > 0) && ($qty_akhir < $row_sales->qty) ) {
											$qty_selisih	=	$row_sales->qty - $qty_akhir;
										}
										if($qty_akhir == 0) {
											$qty_selisih	=	$qty_akhir - $row_sales->qty;
										}
										...
										if($qty_selisih < 0) {
											$total_selisih	=	($qty_selisih * -1) * $unit_cost;	
										} else {
											$total_selisih	=	$qty_selisih * $unit_cost;
										}
										
										$grand_qty_akhir = $grand_qty_akhir + $qty_akhir;
										$grand_qty_selisih = $grand_qty_selisih + $qty_selisih;
										$grand_total_selisih = $grand_total_selisih + $total_selisih;
										##----------------/\--------------------
								...
								?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $row_sales->code ?></td>
                                            <td><?php echo $row_sales->barcode ?></td>
                                            <td><?php echo $row_sales->item_name ?></td>
					                    	<td align="right"><?php echo number_format($row_sales->qty,2,".",",") ?></td>
					                    	<td align="right"><?php echo number_format($qty_akhir,2,".",",") ?></td>
					                    	<td align="right"><?php echo number_format($qty_selisih,2,".",",") ?></td>
					                    	<td align="right"><?php echo number_format($unit_cost,2,".",",") ?></td>
					                    	<td align="right"><?php echo number_format($total,2,".",",") ?></td>
					                    	<td align="right"><?php echo number_format($total_selisih,2,".",",") ?></td>
					                    	<td>
							...
							<tr style="font-weight: bold; font-size: 16px; background-color: #c5fec0">
                                <td colspan="5" align="right"><?php echo "Grand Total" ?></td>
                                <td align="right"><?php echo number_format($grand_qty,2,".",",") ?></td>
                                <td align="right"><?php echo number_format($grand_qty_akhir,2,".",",") ?></td>
                                <td align="right"><?php echo number_format($grand_qty_selisih,2,".",",") ?></td>
                                <td align="right">&nbsp;</td>
		                    	<td align="right"><?php echo number_format($grand_total,2,".",",") ?></td>
		                    	<td align="right"><?php echo number_format($grand_total_selisih,2,".",",") ?></td>

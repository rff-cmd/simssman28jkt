...
<script language="javascript">
	function pilih_ekstra(jumlah) { 
		var maksimal = 0; 
		for(i=0; i<jumlah; i++) {
			var pilih = document.getElementById('pilih_'+i).checked;	
			if(pilih == true) {
				maksimal = maksimal + 1;
			}
			
			if(maksimal > 2) {
				alert("Pilhan Ekstrakurikuler maksimal 2");
				$('#pilih_id'+i).html('<input type="checkbox" id="pilih_'+i+'" name="pilih_'+i+'" style="text-align: right;" onclick="pilih_ekstra('+jumlah+')" class="ace" value="1" ><span class="lbl"></span>');
				return false;
			}
		}
		
		/*$('#jumlah_pilihan_id').html('<input type="hidden" id="jumlah_pilihan" name="jumlah_pilihan" value="'+maksimal+'" >');
		var jumlah_pilihan = document.getElementById('jumlah_pilihan').value;
...
<script type="text/javascript">
	...
	function loadHTMLPost2(URL, destination, button, getId){
		...
		//var str = str + '&' + str2 + '&button=' + button;
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$idsemester		=	$_SESSION["idsemester"];
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_siswa_ekstrakurikuler($ref);
					$row_siswa_ekstrakurikuler=$sql->fetch(PDO::FETCH_OBJ);	
					
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="siswa_ekstrakurikuler" id="siswa_ekstrakurikuler" enctype="multipart/form-data" onSubmit="return cekinput('idsiswa,idtahunajaran,idsemster');" >
				...
				<?php if( empty($ref) ) { ?>   	
	            	<div class="form-group">
						...
						<div class="col-sm-4">
							<select name="idsiswa" id="idsiswa" class="chosen-select form-control" onchange="loadHTMLPost2('app/siswa_ekstrakurikuler_ajax.php','siswa_ekstra_pelajaran','getekstra','idsiswa')" >
								<option value=""></option>
								<?php select_siswa_id($row_siswa_ekstrakurikuler->idsiswa); ?>
							</select>
						</div>
					</div>
				<?php } else { ?> 
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Siswa *)</label>
						<div class="col-sm-4">
							<select name="idsiswax" id="idsiswax" class="chosen-select form-control" disabled="" >
								<option value=""></option>
								<?php select_siswa_id($row_siswa_ekstrakurikuler->idsiswa); ?>
							</select>
						</div>
					...
					<input type="hidden" name="idsiswa" id="idsiswa" value="<?php echo $row_siswa_ekstrakurikuler->idsiswa ?>"/>
            	...
            	<div id="siswa_ekstra_pelajaran">
            		<?php
            			if( !empty($ref) ) {
							include("siswa_ekstrakurikuler_pelajaran_update.php");
						}
            		?>
            	</div>

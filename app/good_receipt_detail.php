	...
	?>
		...
		<?php 
			...
			while($row_outbound_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
				...
				$qty_po = number_format($row_outbound_detail->qty_po, 2, '.', ',');
		...
		?>								
			...
			<input type="hidden" id="old_qty_<?php echo $no; ?>" name="old_qty_<?php echo $no; ?>" style="text-align: right; width: 70px" class="form-control" value="<?php echo $qty ?>" >
				...
				<tr> 
					...
					<td align="center">
						<input type="text" id="qty_po_<?php echo $no; ?>" name="qty_po_<?php echo $no; ?>" style="text-align: right; width:100px " class="form-control" readonly="" onkeyup="formatangka('qty_po_<?php echo $no; ?>')" autocomplete="off" value="<?php echo number_format($qty_po,0,'.',',') ?>" >
					</td>
					<td align="center">
						<input type="text" id="qty_<?php echo $no; ?>" name="qty_<?php echo $no; ?>" style="text-align: right; width:100px " class="form-control" onkeyup="formatangka('qty_<?php echo $no; ?>')" autocomplete="off" value="<?php echo number_format($qty_po,0,'.',',') ?>" >
					</td>

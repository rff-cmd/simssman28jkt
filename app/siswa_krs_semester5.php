<?php if($idtingkat == 46 && $_SESSION["semester_id"]==24) { ?>
	<div id="semester5" class="tab-pane in active">
<?php } else { ?>
		...
		<table class="table" class="table table-bordered table-condensed table-hover table-striped" style="width: auto;">
			...
			<tbody>
				<?php 
					...
					//Kelompok Utama 
					$sql = $selectview->get_daftarnilai_raport("SMA", $idtingkat, $row_kelas->idkelas, '24', $_SESSION["idsiswa"], '', $nama_minat, $row_tahunajaran->replid, $nis);
				...
				?>								
						...
						<input type="hidden" id="kode_<?php echo $no ?>" name="kode_<?php echo $no ?>" value="<?php echo $row_counting_detail->kode1; ?>" >
							...
							<td align="center">
								...
								?>
										...
										<?php } else if($idtingkat >= 46) { 
											if( numeric_semester($idtingkat) > 5) {
												echo 'Tuntas';
											}
										} ?>
								<?php
				...
				<?php
					...
					for($i=0; $i < count($a_pelajaran_id); $i++) {
						$sql = $selectview->get_krs('', $row_kelas->idtingkat, '24', $nama_minat, $row_tahunajaran->replid, $idminat, $a_pelajaran_id[$i]);

    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
        	...
        	if($x > 0) {
				...
				$nis			=	str_replace("'","",$column[1]);
				$nisn			= 	str_replace("'","",$column[2]);
				if(substr($nisn,0,1) != "0") {
					$nisn = "00".$nisn;
					echo "nol=".$nisn."<br>";
				}
				$nama			=	$column[3];
				...
				//cek siswa
				$sqlcek 	= 	"select nisn from siswa where nisn='$nisn'";
				$sqlresult	=	$dbpdo->prepare($sqlcek);
				$sqlresult->execute();
				$rowssiswa	=	$sqlresult->rowCount();
				
				if($rowssiswa == 0) {
					$strsql1 = "update siswa set nisn='$nisn' where nis='$nis'";
					//echo $nisn."<br>";

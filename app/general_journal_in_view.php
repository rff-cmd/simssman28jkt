<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo obraxabrix('general_journal_in_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
...
<div class="page-content">
	...
	<?php
		...
		if ($delete == "xm8r389xemx23xb2378e23") {
			...
			$delete2=new delete;
			$delete2->delete_general_journal_in($segmen4);
	?>
			<div class="alert alert-success">
				<strong>Delete Data successfully</strong>
			</div>
            
            <meta http-equiv="Refresh" content="0;url=<?php echo $nama_folder . '/' . obraxabrix('general_journal_in_view'); ?>" />
	<?php
            				...
            				<div class="form-group"> 
								...
								<div class="col-sm-3">
									<select name="status" id="status" class="chosen-select form-control" style="min-width: 178px;" />
										<?php sparepart_status_select($status); ?>
									</select>
								</div>
							</div>
				...
				<div class="col-xs-12">
					...
					<thead>
					  <tr>
						...
						<th>Detail Pemasukan <i class="fa fa-sort"></i></th>
					...
					<?php
						$j = 0;
						$sql=$select->list_general_journal_in('', $from_date, $to_date, $status, $memo, $all);
						while($row_general_journal_in=$sql->fetch(PDO::FETCH_OBJ)){
					...
					?>
					 <tr>
						...
						<td><?php echo $row_general_journal_in->ref ?></td>
						<td><?php echo $row_general_journal_in->date ?></td>
						<td>
							<table class="table" width="100%" border="1" style="border: 1px solid #07a035">
								...
								<?php
									$xn = 0;
									$sqldet = $select->list_general_journal_detail($row_general_journal_in->ref);
								...
								?>
									<tr>
										...
										<td align="right"><?php echo number_format($datadet->debit_amount,0,'.',','); ?>&nbsp;</td>
									</tr>
								<?php
									}
								?>
						...
						<td align="right"><?php echo number_format($row_general_journal_in->total_debit,0,'.',',') ?></td>
						<td align="center">
							<?php if (allowupd('frmgeneral_journal_in')==1) { ?>
								<a href="<?php echo $nama_folder . '/' . obraxabrix('general_journal_in') ?>/<?php echo $row_general_journal_in->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
									<span class="green">
                            ...
                            <?php if (allowdel('frmgeneral_journal_in')==1) { ?>    
                                &nbsp;
								<a href="JavaScript:hapus('<?php echo $row_general_journal_in->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
									<span class="red">

						...
						<table class="table" id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
                                    ...
                                    <th><?php if($lng==1) { echo 'Opening Balance'; } else { echo 'Awal'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Amount Pay'; } else { echo 'Bayar'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Purchase Return'; } else { echo 'Retur Beli'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'Discount'; } else { echo 'Potongan'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'CN'; } else { echo 'CN'; } ?></th>
			                    	
			                    	<th><?php if($lng==1) { echo 'Purchase'; } else { echo 'Pembelian'; } ?></th>
			                    	<th><?php if($lng==1) { echo 'DN/Adjust'; } else { echo 'DN/Adjust'; } ?></th>
							...
							<tbody>
                                <?php			
                                	...
                                	$sql_ob=$selectview->rpt_ap_card_opening_balance($from_date, $vendor_code);
                                	$row_rpt_ob=$sql_ob->fetch(PDO::FETCH_OBJ);
                                	$saldo_awal = $row_rpt_ob->saldo_awal;
                                	
                                	$total_amaount = $saldo_awal;
                                ...
                                <?php	
									...
									$sql=$selectview->rpt_ap_card($from_date, $to_date, $vendor_code);	
									while ($row_rpt_ap=$sql->fetch(PDO::FETCH_OBJ)) {
										
										$invoice_type = "";
										if($row_rpt_ap->invoice_type == "PUR") {
											$invoice_type = "Purchase Return";
										}
										if($row_rpt_ap->invoice_type == "POV") {
											$invoice_type = "Purchase/Pembelian";
										}
										if($row_rpt_ap->invoice_type == "PMT") {
											$invoice_type = "Payment/Pembayaran";
										}
										
										$debit_amount = 0;
                                		$credit_amount = 0;
                                	
										$debit_amount = $row_rpt_ap->debit_amount;
										$credit_amount = $row_rpt_ap->credit_amount;
										
										$total_amaount = $total_amaount + $credit_amount - $debit_amount;

<?php if($idtingkat == 28 && $_SESSION["semester_id"]==20) { ?>
	<div id="semester4" class="tab-pane in active">
<?php } else { ?>
			...
			<tbody>
				...
				<?php				 
						...
						while($row_counting_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
							...
							//cek mapel semester sebelumnya
							$sqlkls = $selectview->list_riwayatkelassiswa($nis);
							$datakls= $sqlkls->fetch(PDO::FETCH_OBJ);
							$old_idkelas = $datakls->idkelas;
							
							//jika siswa jalur masuk mutasi
							if($old_idkelas == "") {
								$sqlkls = $selectview->list_siswa($nis);
								$datakls= $sqlkls->fetch(PDO::FETCH_OBJ);
								$old_idkelas = $datakls->idkelas;
								$jalur_masuk = $datakls->jalurmasuk;			
							}
							
							$jumlah_nilai = 0;
							$sqlcek=$selectview->krs_pelajaran_cek($_SESSION["idsiswa"], $row_counting_detail->pelajaran_id, 20, $old_idkelas); 
				...
				?>								
						...
						<tr style="background-color:ffffff;" > 
							...
							<td align="center">
								...
								<?php } else if($idtingkat >= 28) { ?>
									<?php 
										if( numeric_semester($idtingkat) > 4) {
											echo 'Tuntas';										
										} ?>
				...
				<?php 
						$no++; 
						} else if ($jalur_masuk == "Mutasi") { //jika siswa jalur masuk mutasi

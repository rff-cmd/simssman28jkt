<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('setup_periode_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_setup_periode($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('setup_periode_view') ?>" />
			<?php
				...
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="jam_view" id="jam_view" enctype="multipart/form-data" >
		            	...
		            	<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Jenis Periode</label>
							<div class="col-sm-3">
								<select name="jenis" id="jenis" class="chosen-select form-control" >
									<option value=""></option>
									<?php select_jenis_periode($jenis); ?>
								</select>							
							</div>
						</div>
						...
						<table class="table" id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th style="font-weight:bold ">Jenis Periode &nbsp;&nbsp;</th>
									<th style="font-weight:bold ">Tanggal Aktif &nbsp;&nbsp;</th>
									<th style="font-weight:bold ">s/d Tanggal &nbsp;&nbsp;</th>
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_setup_periode("", $jenis, $all);
						            while($setup_periode_view=$sql->fetch(PDO::FETCH_OBJ)){
            							
            							$i++;
            							
            							$jenis = $setup_periode_view->jenis;
            							if($jenis == "KRS") {
											$jenis = "KRS";
										}
										if($jenis == "RAPORT") {
										...
										if($jenis == "RAPORT_PTS") {
											$jenis = "Input Rapor PTS";
										}
										if($jenis == "DAFUL") {
											$jenis = "Daftar Ulang Siswa Baru";
										}
										if($jenis == "RAPORT_SISWA") {
											$jenis = "Akses Rapor Siswa";
										}
								...
								?>
                                        ...
                                        <tr>
											...
											<td><?php echo date("d-m-Y", strtotime($setup_periode_view->tanggal)) ?></td>
											<td><?php echo date("d-m-Y", strtotime($setup_periode_view->tanggal1)) ?></td>
											<td><?php echo $setup_periode_view->tingkat ?></td>
											<td>
												<?php if ($setup_periode_view->aktif == 1) { ?>
													<button class="btn btn-xs btn-success">
														<i class="ace-icon fa fa-check bigger-120"></i>
													</button>
												<?php } ?>
											</td>
											<td><?php echo $setup_periode_view->uid ?></td>
											<td><?php echo $setup_periode_view->dlu ?></td>								
											<td align="center">
                                            
                                                <?php if (allowupd('frmsetup_periode')==1) { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('setup_periode') ?>/<?php echo $setup_periode_view->replid ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmsetup_periode')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $setup_periode_view->replid ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">

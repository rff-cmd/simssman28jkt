...
<script type="text/javascript">
	...
	function loadHTMLPost3(URL, destination, button, getId, getId2, getId3){
		...
		var str = str + '&button=' + button + '|' + getId3;
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_guru_ekskul.php");
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_guru_ekskul($ref);
					$row_guru_ekskul=$sql->fetch(PDO::FETCH_OBJ);	
					
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="guru_ekskul" id="guru_ekskul" enctype="multipart/form-data" onSubmit="return cekinput('idpelajaran');" >
            	...
            	<div class="form-group">
					...
					<?php if($ref == "") { ?>
						<div class="col-sm-4">
							<select name="idpelajaran" id="idpelajaran" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/guru_ekskul_ajax.php','guru_pelajaran','geteguru','idpelajaran')" >
								<option value=""></option>
								<?php 
									//select_pelajaran_all($row_guru->idpelajaran); 
									select_ekstrakurikuler($row_guru_ekskul->idpelajaran);
								?>
							</select>
						</div>
					<?php } else { ?>
						<div class="col-sm-4">
							<select name="idpelajaran2" id="idpelajaran2" class="chosen-select form-control" disabled="" onchange="loadHTMLPost2('<?php echo $__folder ?>app/guru_ekskul_ajax.php','guru_pelajaran','geteguru','idpelajaran')" >
								<option value=""></option>
								<?php 
									//select_pelajaran_all($row_guru->idpelajaran); 
									select_ekstrakurikuler($row_guru_ekskul->idpelajaran);
						...
						<input type="hidden" name="idpelajaran" id="idpelajaran" value="<?php echo $row_guru_ekskul->idpelajaran ?>" />
					<?php } ?>
				</div>
				
            	<div id="guru_pelajaran">
            		<?php
            			
            			if( !empty($ref) ) {
							include("guru_ekskul_pelajaran_update.php");
						}
            		?>
            	</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmguru_ekskul')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmguru_ekskul')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmguru_ekskul')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('guru_ekskul_view') ?>'" />

...
<script>
    ...
    {
		if(tipe == 'excel') {
			$("#daftarnilai_input").attr('action', '../app/daftarnilai_pts_xls.php')
			   .attr('target', '_BLANK');
			$("#daftarnilai_input").submit();
		}
		
		if(tipe == 'upload') {
			$("#daftarnilai_input").attr('action', '../app/daftarnilai_pts_upload.php')
			   .attr('target', '_BLANK');
			$("#daftarnilai_input").submit();
		}
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include($__folder."exec/insert_daftar_nilai_pts.php"); 
			...
			<form class="form-horizontal" role="form" action="" method="post" name="daftarnilai_input" id="daftarnilai_input" enctype="multipart/form-data" >
				...
				<?php if( ($protection->setup_periode("RAPORT_PTS", $date) == 1 || allowlvl("frmdaftarnilai") == 2) && $input_nilai == 1) { ?>
					<div class="form-group">
				...
				<table class="table" id="simple-table" class="table table-striped table-bordered table-hover">
					<thead>
						<?php
							...
							if($iddasarpenilaian == 3) {
								$jumlah_ukbm= 7; //$dataukbm->jumlah_ukbm;
							}
							if($iddasarpenilaian == 4) {
								$jumlah_ukbm= 5; 
							}
							if($iddasarpenilaian == 7) {
								$jumlah_ukbm= 6; 
							}
                        ...
                        <?php			
                        	...
                        	if(allow_guru_kelas('', $_SESSION['idguru'], $idtingkat, $idkelas) != "" || $_SESSION['adm'] == 1) {
								...
								$i++;
									...
									$sqlnilai 	= $select->list_daftarnilai_pts($siswa_view->replid, $idtingkat, $idkelas, $idtahunajaran, $semester_id, $idpelajaran, $iddasarpenilaian, $siswa_view->nis);
							...
							?>
	                                ...
	                                <tr style="font-size: 11px">
										...
										<?php
												...
												$b = 1;
												$sqlndetail = $select->list_daftarnilai_pts_detail($siswa_get->replid);
										...
										<td><input type="text" style="width:50px; height: 30px; text-align: center;"  name="hadir<?php echo $i ?>" id="hadir<?php echo $i ?>" onkeyup="formatangka('hadir<?php echo $i ?>'),detailvalue('<?php echo $i ?>', '<?php echo $jumlah_ukbm ?>','<?php echo $idsiwa ?>')" value="<?php echo $siswa_get->hadir ?>"/></td>
									</tr>

...
<?php
if($_POST["submit"]) {
	$sqlsiswa = "select replid, nama from pelajaran where aktif=1 and sifat<>0 order by nama";
	...
	while ($row_siswa = $resultssiswa->fetch(PDO::FETCH_OBJ)){
		...
		if($replid != "") {
			...
			$sqlcek 	= 	"select idpelajaran from m_mapel where idpelajaran='$replid'";
			$sqlresult	=	$dbpdo_uji->prepare($sqlcek);
			$sqlresult->execute();
			$rows = $sqlresult->rowCount();
			
			if($rows == 0) {
				$sqlstr = "insert into m_mapel (idpelajaran, nama) values ('$replid', '$nama')";
				$sql=$dbpdo_uji->prepare($sqlstr);
				$sql->execute();
				
				$ekskul++;
			} else {
				$sqlstr = "update m_mapel set nama='$nama' where idpelajaran='$replid'";
				$sql=$dbpdo_uji->prepare($sqlstr);
				$sql->execute();

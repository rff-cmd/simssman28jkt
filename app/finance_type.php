	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='name') {
			alert('Tipe Keuangan cannot empty!');				
		  }
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('Y-m-d'), 'frmfinance_type', '', '', ''); 
					
				include("app/exec/finance_type_insert.php"); 
				
				if ($ref != "") {
					$sql=$select->list_finance_type($ref);
					$row_finance_type=$sql->fetch(PDO::FETCH_OBJ);	
					
					$ref2	=	$row_finance_type->code;	
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="finance_type" id="finance_type" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('code,name,type');" >
                ...
                <div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Name'; } else { echo 'Jenis Keuangan'; } ?> *)</label>
					<div class="col-sm-3">
						<input type="text" id="name" name="name" class="form-control" autocomplete="off" value="<?php echo $row_finance_type->name ?>">
					</div>
				</div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <select id="type" name="type" class="chosen-select form-control" style="width: auto">
                        <option value=""></option>
                        <?php 
                        	select_finance_type_master($row_finance_type->type);
                        ?>	                            
                      </select>
					</div>
				</div>
				...
				<?php /*
				<div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Location'; } else { echo 'Lokasi/Cabang'; } ?></label>
                    <div class="col-sm-3">
                      <select id="location_id" name="location_id" class="chosen-select form-control" style="width: auto">
                        <option value=""></option>
                        <?php 
                        	combo_select_active("warehouse","id","name","active","1",$row_finance_type->location_id) 
                        ?>	                            
                      </select>
					</div>
				</div>*/ ?>
				...
				<div class="form-group"> 
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">No Account</label>
					<div class="col-sm-5">
						
						<select name="account_code" id="account_code" style="min-width: 178px;" class="chosen-select form-control" >
							<option value=""></option>
							<?php combo_select_active4("coa","syscode","name","acc_code",$row_finance_type->account_code); ?>
						</select>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						...
						<?php } else { ?>
							<input class="ace" type="checkbox" name="active" id="active" value="1" <?php if($row_finance_type->active==1) echo "checked";?>/>
						<?php } ?>
						<span class="lbl"></span>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowadd('frmfinance_type')==1) { ?>
    						<?php if($ref=='') { ?>
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('finance_type_view') ?>'" />

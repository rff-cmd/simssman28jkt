	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='tanggal1') {
			alert('s/d Tanggal tidak boleh kosong!');				
		  }
		  
		  if (document.getElementById(arrf[i]).name=='jenis') {
			alert('Jenis Setup tidak boleh kosong!');				
		  }
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_setup_siswa_khusus.php");
				...
				if ($ref != "") {
					$sql=$select->list_setup_siswa_khusus($ref);
					$setup_siswa_khusus_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$tanggal = date("d-m-Y", strtotime($setup_siswa_khusus_data->tanggal));
					$tanggal1 = date("d-m-Y", strtotime($setup_siswa_khusus_data->tanggal1));
					
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="setup_siswa_khusus2" id="setup_siswa_khusus2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('idsiswa,tanggal,tanggal1,jenis');" >
            	
            	<input type="hidden" id="id" name="id" value="<?php echo $ref ?>" >
            	
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Jenis Setup *)</label>
					<div class="col-lg-2">
						<select name="jenis" id="jenis" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_jenis_periode($setup_siswa_khusus_data->jenis); ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select name="idsiswa" id="idsiswa" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_siswa_id($setup_siswa_khusus_data->idsiswa); ?>
						</select>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Active Date'; } else { echo 'Tanggal Aktif'; } ?> *)</label>
					<div class="col-sm-3">
						<input type="text" id="tanggal" name="tanggal" style="font-size: 12px" class="form-control date-picker" data-date-format="dd-mm-yyyy" value="<?php echo $tanggal ?>">								
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="tanggal1" name="tanggal1" style="font-size: 12px" class="form-control date-picker" data-date-format="dd-mm-yyyy" value="<?php echo $tanggal1 ?>">								
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmsetup_siswa_khusus')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmsetup_siswa_khusus')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmsetup_siswa_khusus')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('setup_siswa_khusus_view') ?>'" />
